...
TEST(test_reflection_extended, test_function_descriptor)
{
    ...
    krrs::reflect::for_each<mocks::with_functions>([&]<typename D>() mutable {
        if constexpr (std::is_function_v<typename D::member_type>)
        {
            ...
            if constexpr (D::name == "add")
            {
                ...
                static_assert(krrs::reflect::typelist_size_v<args> == 2);
                static_assert(std::same_as<krrs::reflect::typelist_element_t<0, args>, int>);
                static_assert(std::same_as<krrs::reflect::typelist_element_t<1, args>, int>);

                auto fn = krrs::reflect::get_member_variable<D>(obj);
                EXPECT_EQ(fn(3, 4), 7);
                EXPECT_EQ(fn(-1, 1), 0);
            }
            else if constexpr (D::name == "scale")
            {
                ...
                static_assert(krrs::reflect::typelist_size_v<args> == 1);
                static_assert(std::same_as<krrs::reflect::typelist_element_t<0, args>, double>);
            ...
            else if constexpr (D::name == "reset")
            {
                ...
                static_assert(krrs::reflect::typelist_size_v<args> == 2);
                static_assert(std::same_as<krrs::reflect::typelist_element_t<0, args>, int>);
                static_assert(std::same_as<krrs::reflect::typelist_element_t<1, args>, double>);
            ...
            else if constexpr (D::name == "fill_buffer")
            {
                ...
                static_assert(std::same_as<ret, void>);
                static_assert(krrs::reflect::typelist_size_v<args> == 1);
                static_assert(std::same_as<krrs::reflect::typelist_element_t<0, args>, std::array<int, 3>>);
        ...
        else
        {
            ...
            else if constexpr (D::name == "tags")
            {
                ...
                const auto& t = krrs::reflect::get_member_variable<D>(obj);
                EXPECT_EQ(t.size(), 3u);
                EXPECT_EQ(t[0], "alpha");
            }
            else if constexpr (D::name == "registry")
            {
                static_assert((std::same_as<typename D::member_type, std::unordered_map<std::string, double>>));
                const auto& reg = krrs::reflect::get_member_variable<D>(obj);
            ...
            else if constexpr (D::name == "maybe_status")
            {
                ...
                const auto& ms = krrs::reflect::get_member_variable<D>(obj);
...
TEST(test_reflection_extended, test_descriptor_for)
{
    ...
    using desc_l = krrs::reflect::descriptor_for<mocks::foo, &mocks::foo::l>;
    using desc_i = krrs::reflect::descriptor_for<mocks::foo, &mocks::foo::i>;
    using desc_s = krrs::reflect::descriptor_for<mocks::foo, &mocks::foo::s>;
    using desc_c = krrs::reflect::descriptor_for<mocks::foo, &mocks::foo::c>;
    ...
    using desc_price = krrs::reflect::descriptor_for<mocks::bar, &mocks::bar::price>;
    ...
    using desc_tag = krrs::reflect::descriptor_for<mocks::bar, &mocks::bar::tag>;
    ...
    static_assert(krrs::reflect::concepts::descriptor_like<desc_l>);
    static_assert(krrs::reflect::concepts::descriptor_like<desc_price>);

    // lookup works for complex-typed data members
    using desc_tags = krrs::reflect::descriptor_for<mocks::with_functions, &mocks::with_functions::tags>;
    using desc_registry = krrs::reflect::descriptor_for<mocks::with_functions, &mocks::with_functions::registry>;
    using desc_maybe = krrs::reflect::descriptor_for<mocks::with_functions, &mocks::with_functions::maybe_status>;
    ...
    using desc_add = krrs::reflect::descriptor_for<mocks::with_functions, &mocks::with_functions::add>;
    ...
    static_assert(krrs::reflect::concepts::descriptor_like<desc_add>);

    using desc_find = krrs::reflect::descriptor_for<mocks::with_functions, &mocks::with_functions::find_in_buffer>;
...
TEST(test_reflection_extended, test_derived)
{
    ...
    krrs::reflect::for_each<mocks::base>([&]<typename Descriptor>() { ++base_count; });
    EXPECT_EQ(base_count, 5u);

    // base_2 has 3 members: weight, priority, active
    std::size_t base_2_count = 0;
    krrs::reflect::for_each<mocks::base_2>([&]<typename Descriptor>() { ++base_2_count; });
    ...
    krrs::reflect::for_each<mocks::derived_more>([&]<typename Descriptor>() { ++total_count; });
    ...
    using desc_x = krrs::reflect::descriptor_for<mocks::derived_more, &mocks::derived_more::x>;
    using desc_kind = krrs::reflect::descriptor_for<mocks::derived_more, &mocks::derived_more::kind>;
    using desc_note = krrs::reflect::descriptor_for<mocks::derived_more, &mocks::derived_more::note>;
