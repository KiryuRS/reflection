...
namespace krrs::argparse::convertors {
...
{
    const std::string lower = value | std::views::transform([](char c) -> char { return static_cast<char>(std::tolower(c)); }) | std::ranges::to<std::string>();
    if (lower == "true")
    {
        return true;
    }
...
template <::krrs::reflect::concepts::stringable T>
constexpr std::optional<T> parse_value(std::string_view value)
{
    return T{value};
}
...
constexpr std::optional<T> parse_value(std::string_view value)
{
    ...
    auto parsed_values = value | std::views::split(","sv) | std::views::transform([&parsed_fail, &default_value](auto subrange) -> value_type {
